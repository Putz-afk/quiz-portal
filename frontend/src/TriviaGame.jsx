...
import { Trophy, Users, AlertCircle, CheckCircle, XCircle, ArrowRight, WifiOff, Crown, RefreshCw, Copy } from 'lucide-react';

const API_URL = "https://lady-unexcogitative-supremely.ngrok-free.dev";
const WS_URL = API_URL.replace(/^http/, 'ws') + "/ws";

export default function TriviaGame({ onBackToMenu }) {
  useEffect(() => {
    document.title = 'Trivia Quiz - QuizPortal';
  }, []);
  ...
  const submitAnswer = (index) => {
    ...
    submittedAnswerRef.current = index;
    socketRef.current.send(JSON.stringify({
    ...
    return (
      <div style={styles.container} className="quiz-app-root">
        <div style={styles.card} className="quiz-card">
          <h1 style={styles.title}><Users size={32} /> Trivia Quiz</h1>
          ...
          <button style={{...styles.secondaryBtn, marginTop: 10, background: '#e0e7ff', color: '#667eea'}} onClick={handleBackToMenu}>
            Back to Menu
          </button>
        </div>
      </div>
    ...
    return (
      <div style={styles.container} className="quiz-app-root">
        <div style={styles.card} className="quiz-card">
          ...
          <button style={{...styles.secondaryBtn, marginTop: 10}} onClick={handleBackToMenu}>
            Back to Menu
          </button>
        </div>
      </div>
...
const styles = {
  ...
  badge: { display: 'inline-block', background: '#e0e7ff', color: '#667eea', padding: '4px 12px', borderRadius: '12px', fontSize: '0.9rem', fontWeight: 'bold' },
  label: { textAlign: 'left', marginBottom: 10, color: '#333', fontWeight: '500', fontSize: '0.95rem' },
